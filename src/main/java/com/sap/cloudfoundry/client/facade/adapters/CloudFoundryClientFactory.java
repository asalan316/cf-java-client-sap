...
public abstract class CloudFoundryClientFactory {
  ...
  public CloudFoundryClient createClient(
      URL controllerUrl, OAuthClient oAuthClient, Map<String, String> requestTags) {
  ...
  public LogCacheClient createLogCacheClient(
      URL controllerUrl, OAuthClient oAuthClient, Map<String, String> requestTags) {
    String logCacheApi;
    try {
      ...
      LOGGER.warn(
          MessageFormat.format(
              Messages.CALL_TO_0_FAILED_WITH_1, controllerUrl.toString(), e.getMessage()),
          e);
      logCacheApi = controllerUrl.toString().replace("api", "log-cache");
    }
  ...
  private Map<String, Object> callCfRoot(URL controllerUrl, Map<String, String> requestTags) {
    ...
    try {
      ...
      LOGGER.info(
          MessageFormat.format(Messages.CALLING_CF_ROOT_0_TO_ACCESS_LOG_CACHE_URL, controllerUrl));
  ...
  private HttpRequest buildCfRootRequest(URL controllerUrl, Map<String, String> requestTags)
      throws URISyntaxException {
    var requestBuilder =
        HttpRequest.newBuilder().GET().uri(controllerUrl.toURI()).timeout(Duration.ofMinutes(5));
    requestTags.forEach(requestBuilder::header);
    return requestBuilder.build();
  }

  public CloudSpaceClient createSpaceClient(
      URL controllerUrl, OAuthClient oAuthClient, Map<String, String> requestTags) {
    String v3Api;
    try {
      var links = CloudUtil.executeWithRetry(() -> callCfRoot(controllerUrl, requestTags));
  ...
  private SpacesV3 createV3SpacesClient(
      URL controllerUrl, String v3Api, OAuthClient oAuthClient, Map<String, String> requestTags) {
    return new ReactorSpacesV3(
        getOrCreateConnectionContext(controllerUrl.getHost()),
        Mono.just(v3Api),
        oAuthClient.getTokenProvider(),
        requestTags);
  }

  private OrganizationsV3 createV3OrgsClient(
      URL controllerUrl, String v3Api, OAuthClient oAuthClient, Map<String, String> requestTags) {
    return new ReactorOrganizationsV3(
        getOrCreateConnectionContext(controllerUrl.getHost()),
        Mono.just(v3Api),
        oAuthClient.getTokenProvider(),
        requestTags);
  ...
  public ConnectionContext getOrCreateConnectionContext(String controllerApiHost) {
    ...
    // return connectionContextCache.computeIfAbsent(controllerApiHost,
  ...
  private ConnectionContext createConnectionContext(String controllerApiHost) {
    ...
    DefaultConnectionContext.Builder builder =
        DefaultConnectionContext.builder().skipSslValidation(true).apiHost(controllerApiHost);
  ...
  private reactor.netty.http.client.HttpClient getAdditionalHttpClientConfiguration(
    ...
    clientWithOptions =
        clientWithOptions.secure(
            t -> {
              final SslContext sslContext;
              try {
                sslContext =
                    SslContextBuilder.forClient()
                        .trustManager(FascadeSSLUtil.NULL_TRUST_MANAGER)
                        .build();
                t.sslContext(sslContext);
              } catch (SSLException e) {
                LOGGER.info("custom-test: error creatign ssl context", e);
